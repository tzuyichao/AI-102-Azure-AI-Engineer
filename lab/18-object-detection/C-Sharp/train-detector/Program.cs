...
namespace train_detector
{
    class Program
    {
        ...
        static void Main()
        {
            try
            {
                Console.WriteLine("Upload Images to custom vision - object detection project");
                ...
                trainingClient = new CustomVisionTrainingClient(new ApiKeyServiceClientCredentials(trainingkey))
                {
        ...
        static void UploadImages(string folder)
        {
            ...
            ImageCreateSummary result = trainingClient.CreateImagesFromFiles(customVisionProject.Id, new ImageFileCreateBatch(imageFileEntries));
